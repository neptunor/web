<div align="center">
  ...
  <p>
    <em>Bright Ocean Studio × Neptunor Marine — a complete custom RIB & inflatable boat manufacturing platform built on the Vectoflare full-stack SaaS starter.</em>
  </p>
</div>
...
| `dictionary/en-product.ts` | Product-specific strings (legal, inquiry, boat-specific) | Your product text |
...
Replace all `neptunor` prefixes with your `SITE_ID`:
- `name`: `myproduct`
...
This guide maps the **major architectural layers** of Neptunor to their source files, so you can jump straight to the code that matters.
...
Neptunor is the production-ready web presence for a RIB boat & inflatable boat OEM/ODM manufacturer — the marketing site is positioned as a **custom boat development & manufacturing partner**, not a "launch your own brand" tool. It pairs a fully designed bilingual (en / es) marketing site with the complete SaaS backend from [Vectoflare](https://github.com/vectoflare/vectoflare): auth, email, an admin console, and more — every feature a real implementation, no mocks or stubs, running on the Cloudflare free-to-cheap stack (Workers + D1 + KV + R2). The full Neptunor brand content (factory, technology, research, news, product pages) is served from the same Worker, English-first, under `/` with Spanish at `/es`.
...
| **Site search** | Two surfaces: a header search dialog fed by `/search-index.json` (the full public index, edge-cached 1h) and a `/search` page with a full-text (Orama) index built from the same data — solutions, knowledge hub, projects, product series, Neptunor pages, news, technology, case studies, guides, FAQs and the six hub/landing pages (home, /products, /solutions, /projects, /knowledge, /gallery) in en+es |
...
| **Legacy SEO landings** | Retired keyword pages (`/oem-boat-moq` consolidations, old SUP-era URLs) are handled by the **edge URL gate**: 301 merges to their new equivalents, 410 for removed template pages — search equity preserved, one source of truth |
...
**AI-ready content**: `/llms.txt` and `/llms-full.txt` index the docs, the full product catalog (names, SKUs, specs, recommended use), the 5 solution pages with their FAQ, and the whole Neptunor brand corpus (factory facts, technology, research, news) — so answer engines can cite the actual offering. `/entity.json` exposes the schema.org Organization entity behind the factory, and `/rss.xml` the news feed. Every entry carries a **page-level meta spec** enforced at build time: `title ≤ 70` chars, `description 80–170` chars (bilingual, en/es).
...
| **Dev/Ops** | Cron Triggers (daily maintenance cleanup at 03:00 UTC + edge-cache warming every 5 min), local/staging/prod environment separation, GitHub Actions CI (lint + typecheck + test + build) and a full deploy pipeline (migrate → deploy → purge CDN → warm edge → sync secrets → upload R2 site assets). |
  ...
  features/          # ★ WEBSITE FOUNDATION — framework code, no product knowledge
   ...
   routes/
     {-$locale}/      Locale-prefixed pages (/, /es, /products, /solutions, etc.)
       ...
       solutions/     5 solution pages (custom-boats, private-label-boats, fleet-rental, rescue-professional, yacht-tender-dealers)
...
> R2 (object storage) is enabled by default in `wrangler.jsonc` and wired into the code (avatar upload reference). Before deploying, create the bucket: `wrangler r2 bucket create neptunor-files` (see [storage](src/content/docs/features/storage.mdx)).

## GitHub Actions & deployment configuration

The repo ships four workflows:
...
|----------|----------|---------------|
| `CF_PROD_D1_ID` | Yes | Production D1 database id — Cloudflare Dashboard → Workers & Pages → D1 → open database `neptunor-db-prod` → copy `Database ID`; or `wrangler d1 list` (needs the token's D1 read permission). The deploy **fails** without it |
...
| `CF_PROD_DOMAIN` | No | Custom domain, e.g. `neptunor.com`. When set, the deploy binds it as a custom-domain route on the production Worker (also makes the CDN purge + warm steps meaningful) |
...
| `BETTER_AUTH_URL` | Production URL, e.g. `https://neptunor.com` (also drives canonical/sitemap origin) | **Required** |
| `RESEND_API_KEY` | [resend.com](https://resend.com) → API Keys → create a key | Emails captured to console |
| `EMAIL_FROM` | e.g. `Neptunor <noreply@neptunor.com>` — **verify your domain in Resend first** (Settings → Domains) | No sender |
...
Create it at [dash.cloudflare.com/profile/api-tokens](https://dash.cloudflare.com/profile/api-tokens) → **Create Token** → **Create Custom Token**. Give it a name (e.g. `neptunor-ci`), set the lifetime (≤ 1 year recommended), and tick **both** the Account resources and the Zone resources below. Then create the token **once** and copy it into the `CLOUDFLARE_API_TOKEN` GitHub secret — it is shown only a single time.
...
| D1 | Edit | `d1 migrations apply neptunor-db-prod --env production --remote` before every deploy; `wrangler d1 list` | Yes |
| R2 Storage | Edit | creates buckets + uploads site media (`scripts/upload-site-assets.mjs`, run by the deploy pipeline before deploy) | Yes |
| Vectorize | Edit | idempotently creates the 3 knowledge indexes (`neptunor-knowledge`, `-staging`, `-prod`) before deploy — deploy fails with code 10159 if the bound index is missing | Yes |
| Account Settings | Read | wrangler account/plan diagnostics | Recommended |

**Zone resources** (scope to the zone `neptunor.com`):

| Resource | Permission | Why the pipeline needs it | Required |
|----------|-----------|---------------------------|----------|
| Zone | Read | `GET /zones?name=neptunor.com` — resolves the zone id that the purge and R2-custom-domain steps target | Yes (with a custom domain) |
