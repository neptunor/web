# Neptunor — LLM / RAG Content Index
...
|----------|--------|---------------|-------------|
| `/llms.txt` | `src/features/site/llm.ts` + `src/features/docs/llm.ts` | Markdown index | Products + solution pages + brand content index (technology, research, news, product pages, geo facts) |
...
| `/rss.xml` | site news posts (RSS feed) | XML | Latest news/posts from the news corpus |
...
| `/sitemap.xml` | `src/features/seo/seo.ts` (`PUBLIC_PATHS` × locales + content registry) | XML | Bilingual entries with hreflang; single-locale entries for single-locale pages |
...
export function buildLlmIndex(locale: 'en' | 'es' = 'en') {
  ...
  chunks.push(...buildContentChunks(locale));
...
- **Chunk ids** are stable FNV-1a hashes of `(locale, url, part)` → daily re-runs upsert in place in Vectorize `neptunor-knowledge` / `-staging` / `-prod`. These indexes only exist when the `vectorize` block is uncommented in `wrangler.jsonc`.
