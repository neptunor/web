...
- **AI sales assistant** (`src/features/ai/`) — a floating RAG chat widget on the
  ...
  brand products/news/technology/case-studies/pages, and every site FAQ as its
...
- **Site search** — three surfaces over one index: a header search dialog fed by
  ...
  pages, knowledge hub, projects, product series, brand products/news/technology/
...
- **AEO/GEO surfaces brought current** — `entity.json` now serves a
  site-specific `description` (plant scale, MOQ tiers, lead times) and a
...
- **Whole-site audit fixes** —
  - ``/about`` registry slug casing mismatch between registry and YAML files on
    disk — slug corrected.
  ...
  - Stale standalone claims: `numberOfEmployees` 200+ → 350+ in brand JSON-LD
    ...
    old brand-only framing, case-study hub intro naming and RSS descriptions —
    all brought to the current brand voice; removed dead ``trackEngage`` and the
    committed ``cf-inspect.log``.
...
- **Deploy pipeline** — CDN cache purge after every release (token now carries
  `Zone → Cache Purge`), edge-cache warming and R2 site-asset upload steps.
...
[Unreleased]: https://github.com/neptunor/web/compare/v1.1.0...main
[1.1.0]: https://github.com/neptunor/web/releases/tag/v1.1.0
[1.0.0]: https://github.com/neptunor/web/releases/tag/v1.0.0
