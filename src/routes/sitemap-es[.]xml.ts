...
// es pages and detail content (news/products/technology/case-use/guides)
// that ship a real Spanish variant.
const handler = () => {
  const origin = new URL(env.BETTER_AUTH_URL).origin
  const contentEs = getLocalePaths('es')
  ...
  return new Response(buildLocaleSitemap(origin, 'es', [...PUBLIC_PATHS, ...detailEs, ...contentEs]), {
