...
const SERVICE_SCHEMA_PAGES: Record<string, { serviceType: string; description: string }> = {
  ...
  '/factory/oem-capability': { serviceType: 'Factory OEM Capability', description: 'Factory OEM capabilities: production lines, CNC precision, RF welding, quality gates, and capacity for custom boat manufacturing.' },
  ...
  '/oem-boat-moq': { serviceType: 'OEM Boat MOQ & Lead Time', description: 'Inflatable boat OEM minimum order quantities and lead times — per-configuration MOQ, tooling timelines, and sample turnaround.' },
  ...
  '/inflatable-boat-certification': { serviceType: 'Inflatable Boat Certification', description: 'Certification and compliance guide: CE (EU RCD 2013/53/EU), ISO 6185, ISO 9001, BSCI, REACH, RoHS requirements for inflatable boats.' },
  '/factory-audit-checklist': { serviceType: 'Factory Audit Checklist', description: 'Eight-area factory audit checklist for boat buyers: quality system, capacity, traceability, pressure testing, welding controls, social compliance, documentation, and post-delivery support.' },
  '/product-development': { serviceType: 'Product Development', description: 'Custom boat product development service — from concept and design through prototyping, testing, and first production run.' },
  '/oem-manufacturing': { serviceType: 'OEM Boat Manufacturing', description: 'OEM boat manufacturing: custom inflatable RIBs, rescue boats and tenders for brand partners.' },
  '/b2b-solutions-matrix': { serviceType: 'B2B Solutions Matrix', description: 'Structured overview of OEM, ODM, private-label, distributor, and fleet-rental partnership models for boat brands.' },
  '/solutions/fleet-rental': { serviceType: 'Fleet Rental Solutions', description: 'Boat equipment programs for rental operators: bulk fleet pricing, custom branding, and durable construction for high-use environments.' },
  '/solutions/private-label-boats': { serviceType: 'Private Label Boat Solutions', description: 'Private-label partnership program for boat dealers: wholesale pricing, display packages, and territory protection.' },
  '/solutions/yacht-tender-dealers': { serviceType: 'Distributor Solutions', description: 'Global boat distribution network: exclusive territories, complete product portfolio, and dedicated account management.' },
}
...
export function ContentCatchAll({ data }: { data: CatchAllData }) {
  return (
    <I18nProvider locale={data.locale}>
      <SiteIndexProvider value={data.index}>
        <MarketingShell>
      ...
      return (
        <>
          ...
          {JSONLD_KEYWORDS[data.path] && (
            <JsonLd
              data={keywordTechArticleLd(data.origin, data.path, data.title, data.description, page, data.locale, JSONLD_KEYWORDS[data.path].keywords, JSONLD_KEYWORDS[data.path].articleTitle)}
            />
          )}
      ...
      <section className="mx-auto max-w-6xl px-5 py-14 md:px-7 md:py-16">
        ...
        <div className="mx-auto mt-12 max-w-3xl">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <a href={fl('/solutions/private-label-boats')} className="marine-card p-5">
