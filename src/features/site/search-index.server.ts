 ...
 * by the `/search` page server fn, so the content corpus it pulls in via the
...
/** Content-layer detail pages (products/news/technology/case-use/guides) for one locale. */
...
/** Content + site-FAQ entries for one locale (the `/search` server filter). */
