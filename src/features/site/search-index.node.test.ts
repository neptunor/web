/**
 * Search index tests (node pool — full site corpus resolves like any module).
...
describe('buildExtendedIndex', () => {
  ...
  test('body-only keywords are searchable (pressure-hold spec)', () => {
    for (const locale of ['en', 'es'] as const) {
      const hits = buildExtendedIndex(locale).filter((it) => (it.content ?? '').includes('0.35 bar'))
...
describe('buildFullIndex', () => {
  ...
  test('deduplicates urls across locales and keeps content', () => {
    ...
    expect(entries.some((it) => (it.content ?? '').includes('0.35 bar'))).toBe(true)
  })
})
