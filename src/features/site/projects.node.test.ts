...
test('flagship case renders metrics strip, takeaways, scenario CTA and related cases', () => {
  ...
  expect(out).toContain('Standardizing a 200-Boat Rental Fleet') // related case card
})
...
test('project without metrics renders no takeaways block', () => {
  const out = html('fishing-boat-program')
