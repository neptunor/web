...
test('干净输入原样保留', async () => {
  await sendInquiryNotification(null, 'from@neptunor.com', ['admin@neptunor.com'], {
    inquiry: inquiry({ company: 'Acme GmbH', email: 'sales@acme.com', requirements: 'Need 500 units' }),
    fileUrl: null,
    origin: 'https://neptunor.com',
  })
  expect(captured).toHaveLength(1)
  const html = captured[0].html
...
test('邮件 Files 行带文件扩展名徽章（从 R2 key 派生，转义后输出）', async () => {
  await sendInquiryNotification(null, 'from@neptunor.com', ['admin@neptunor.com'], {
    ...
    fileUrl: 'https://neptunor.com/api/inquiry-logo/inq-7',
    origin: 'https://neptunor.com',
  })
  expect(captured).toHaveLength(1)
  expect(captured[0].text).toContain('DWG — https://neptunor.com/api/inquiry-logo/inq-7')
