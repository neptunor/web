...
describe('normalizeQuestion', () => {
  test('lowers case, strips punctuation, keeps letters/numbers', () => {
    expect(normalizeQuestion('  MOQ, for RIB-boats?  ')).toBe('moq for rib boats')
  })
})
...
describe('matchFaq', () => {
  const faqs = [
    { q: 'Where is your company and factory located?', a: 'We are based in Qingdao, China. An 18,000 m² inflatable boat manufacturing plant, producing since 2012.' },
    { q: 'What is the minimum order quantity (MOQ) for OEM inflatable boats?', a: 'Minimum order quantity is 90–100+ pieces per standard production run.' },
    { q: 'How long does a sample take?', a: 'Samples ship in 10–15 days.' },
    { q: 'What is the lead time for OEM production?', a: 'Standard OEM production lead time is 30–45 days from confirmed PO.' },
  ...
  test('Chinese question about lead time matches English FAQ', () => {
    ...
    expect(hit).not.toBeNull()
    expect(hit!.answer).toContain('30–45')
  })
})

describe('matchCorpus', () => {
  const chunks = [
    { id: '1', text: 'MOQ for standard volume production starts at 90–100+ pcs. Pilot batches from 20–50 pcs.', url: '/oem-boat-moq', title: 'Custom Boat MOQ' },
    { id: '2', text: 'We hold BSCI, ISO 9001, CE (RCD 2013/53/EU), REACH and RoHS certifications for our Qingdao factory.', url: '/inflatable-boat-certification', title: 'Inflatable Boat Certification Guide' },
    { id: '3', text: 'Sample lead time is 10–15 days after artwork confirmation. Production is 30–45 days from confirmed PO.', url: '/faq', title: 'FAQ' },
  ]
  test('matches corpus chunk by keyword overlap', () => {
    const hit = matchCorpus('What certifications do you have?', chunks)
    expect(hit).not.toBeNull()
    expect(hit!.chunk.url).toBe('/inflatable-boat-certification')
...
describe('buildChunks', () => {
  ...
  test('yaml page section text is chunked (e.g. factory capacity numbers)', () => {
    const chunks = buildChunks('en')
    expect(chunks.some((c) => c.text.includes('18,000'))).toBe(true)
  })
