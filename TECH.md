# Neptunor — Technical Documentation

> Last updated: 2026-08-20
> Project path: `E:\github\Neptunor`
> Production: https://neptunor.com (Cloudflare Workers, `neptunor-production`)
> Stack: TanStack Start (React 19) + Cloudflare Workers + D1 (Drizzle ORM) + KV + R2 + better-auth + Resend + Orama (search) + Fumadocs (docs)
> > Media: All large assets (videos, PDFs, quality photos, product photos) migrated to Cloudflare R2 bucket `neptunor-files-prod`, served via CDN `assets.neptunor.com/site/*`; `public/assets/*` directories added to `.gitignore`; upload script `scripts/upload-site-assets.mjs` supports `--prefix <prefix>` for multi-site key isolation.
...
|----------|--------|--------|
| `/sitemap.xml` | `src/features/seo/seo.ts` (`PUBLIC_PATHS` × locales, hreflang alternates) + docs pages + brand public paths (registry + products/news/technology/case-studies/guides) | XML; bilingual entries with hreflang, single-locale entries (no alternates) for single-locale brand pages |
...
| `/rss.xml` | site news posts | RSS feed |
...
The five `/solutions/*` pages (custom-boats, private-label-boats, fleet-rental, rescue-professional, yacht-tender-dealers) are data-driven: `solution-pages.ts` (en/es) + the `solution-page.tsx` renderer + the `solution-route.tsx` route factory, mounted under the `solutions.tsx` layout with a hub at `solutions/index.tsx`. Every page shares one business logic — scenario → problems → solution → process → case study → FAQ — and ends in a **CTA temperature** (`cold` = Learn More, `warm` = Discuss Your Project, `hot` = Request Manufacturing Proposal): custom-boats is hot, private-label-boats and fleet-rental warm, rescue-professional and yacht-tender-dealers cold.

Retired landing routes are stubs whose loaders throw `redirect({ href: localizePath(locale, target), statusCode: 301 })`. Broader URL policy (legacy dups, removed pages, trailing slashes, retired `/zh`) lives in the **edge gate** (`src/features/seo/edge-gate.ts`) so it runs before SSR:

- Old URLs keep their search equity via `EDGE_REDIRECTS`; es requests redirect to the `/es/...` equivalents via `localizePath`.
...
| Site media | `pnpm upload:site-assets` — uploads videos/PDFs/photos to R2 (`--missing` only PUTs absent objects) |
| CI | `ci.yml` (lint + typecheck + test + build, no secrets); `deploy.yml` (push to `main`: gen `wrangler.jsonc` → build with `CLOUDFLARE_ENV=production` → **D1 migrations** → `wrangler deploy` → **purge CDN cache** → **warm edge cache** (`/`, `/es`, products) → `wrangler secret bulk` from GitHub secrets → **upload R2 site assets**). Missing `CLOUDFLARE_API_TOKEN` → whole deploy job skips gracefully. |
...
| Layer | Piece |
|-------|-------|
| Corpus | `corpus.ts` `buildChunks(locale)` — one atomic chunk per piece of info: solution pages (+ their FAQ blocks individually), knowledge hub per-section, projects, product series, guides, brand products/news/technology/case-studies/pages, and every site FAQ as its own Q/A chunk; en + es, URLs localized via `localizePath` |
| Embeddings | Workers AI `@cf/baai/bge-m3` (1024-dim, multilingual) in batches of 64 |
| Index | Vectorize `neptunor-knowledge` / `-staging` / `-prod` (per env); chunk ids are stable FNV-1a hashes of `(locale, url, part)` so daily re-runs upsert in place |
