...
> > Media: Videos, quality photos, download PDFs and context images live in Cloudflare R2 bucket `neptunor-files-prod`, served via CDN `assets.neptunor.com/site/*`. `public/downloads/` and `public/assets/quality/` ARE committed (small footprint, like product photos) so CI prepares + uploads them; `public/assets/videos/` and `public/assets/images/` are git-ignored; all are synced by `scripts/upload-site-assets.mjs` (`--prefix` supported for multi-site key isolation). **Product photos are NOT uploaded to R2** — they are committed under `public/assets/products/*` (~1.4 k files) and served as Worker static assets at `/assets/products/*`, with AVIF/WebP responsive variants generated by `scripts/process-images.mjs` (sharp) and PDFs compressed by `scripts/compress-pdfs.mjs` (Ghostscript) before deploy.
> > Tests: 281 (Vitest node + workers pools, 46 files); `pnpm typecheck` / `pnpm build` green
...
| Assets | Self-hosted fonts, `public/` statics. **Product photos are committed** under `public/assets/products/*` and served as Worker static assets at `/assets/products/*` — no R2/CDN runtime dependency; `scripts/process-images.mjs` (sharp) pre-scales them to a responsive AVIF/WebP width ladder and writes `src/config/responsive-manifest.json`, consumed by `<ResponsiveImg>` (`src/components/ui/responsive-img.tsx`, plain `<img>` fallback for non-product URLs). Videos/PDFs/quality photos/context images live on the R2 CDN `assets.neptunor.com/site/*`. All site content (`src/content/site/`) is bundled at build time via Vite glob + `?raw` — no filesystem at runtime. Catch-all route `$.tsx` renders content via `ContentCatchAll` component (`src/features/content/catchall.tsx`). |
...
|------|---------|
| Dev server | `pnpm dev` |
| Tests | `pnpm test` (Vitest: `*.node.test.ts` = pure logic, `*.workers.test.ts` = D1/R2/KV via CF vitest pool) — 281 tests (46 files) |
...
| Site media | `pnpm upload:site-assets` — uploads videos/PDFs/quality photos to R2 (product photos are committed static assets, not uploaded; `--missing` only PUTs absent objects) |
| CI | `ci.yml` (lint + typecheck + test + build, no secrets); `deploy.yml` (push to `main`: gen `wrangler.jsonc` → **prepare media** (`images:process` sharp + `pdfs:compress` Ghostscript, before build) → build with `CLOUDFLARE_ENV=production` → apply **D1 migrations** → ensure **Vectorize indexes** → **upload site media to R2** (product photos stay committed) → `wrangler deploy` (**5×75s retry** on Cloudflare API throttling code 971) → **purge CDN cache** → **warm edge cache** → `wrangler secret bulk` (**5×75s retry**) ). Missing `CLOUDFLARE_API_TOKEN` → whole deploy job skips gracefully. |
